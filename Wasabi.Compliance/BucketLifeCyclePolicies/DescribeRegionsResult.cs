...
namespace BucketLifeCyclePolicies
{
    ...
    {
		internal async static Task<List<string>> LoadRegionsAsync()
        {
			var describeRegions = "https://s3.us-east-1.wasabisys.com/?describeRegions";
            ...
            using (StringReader reader = new StringReader(await result.Content.ReadAsStringAsync()))
            {
				...
				return test.Item.Select(item => item.Region).OrderBy(i => i).ToList();	
            }
